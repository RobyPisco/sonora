...
class SettingsPage(QWidget):
    ...
    def _page_updates(self) -> QWidget:
        ...
        self.news_btn = QPushButton("Novità")
        self.news_btn.setObjectName("Ghost")
        self.news_btn.setToolTip("Cosa è cambiato nelle varie versioni di Sonora.")
        self.news_btn.clicked.connect(
            lambda: self._main._show_changelog(changelog.CHANGELOG))
        arow = QHBoxLayout()
        arow.addWidget(self.about_btn)
        arow.addWidget(self.news_btn)
        arow.addStretch(1)
        lay.addLayout(arow)
        lay.addStretch(1)
        return card
