...
Versione corrente: **1.5.1** (fix: icone SVG non incluse nel pacchetto PyInstaller — `build.spec` ignorava le sottocartelle di `resources/`, es. `resources/icons/`; 1.5.0: redesign completo UI, vedi sotto; 1.4.0: basi «senza una traccia»; 1.3.0: export stem separati/WAV-MP3 + niente auto-analisi; 1.2.0: menu modalità stem riorganizzato; 1.1.0: Roformer SW 6 stem + pulsanti ±semitono nel Mixer; allineata in `app/__init__.py`, `installer/sonora.iss` e GitHub).
