...
Versione corrente: **1.11.1** (**fix separazione file locali**: separare in stem un file caricato dal PC falliva con `Roformer: FileNotFoundError [WinError 2]` se nella sessione non era mai partito un download/anteprima — il child `audio_separator` non trovava ffmpeg/ffprobe perché `bin/` finiva nel PATH solo come effetto collaterale di `paths.ffmpeg_dir()`, chiamata dal solo flusso download. Ora `stems._env()` prepende sempre `paths.bin_dir()` al PATH del motore, **dopo** lo strip di `_MEIPASS` (sicuro: `bin/` ha solo eseguibili, non DLL 3.14). Fixa tutti i flussi; test nuovi `test_env_prepends_bin_when_ffmpeg_present`/`_no_bin_when_ffmpeg_absent` in `test_stems.py`; 1.11.0: **pre-conteggio dal vivo + stabilità audio**: nuovo pulsante «Conteggio» nel gruppo CLICK del Mixer — al Play il motore suona un count-in a tempo (`MixerEngine.arm_count_in`/`render_count_in`) prima dell'audio senza far avanzare la playhead, con opzione «anche a ogni giro del loop» che inietta il conteggio al wrap del loop (`_loop_countin` template, rigenerato a ogni cambio velocità/beat); UI in `ui_mixer.py`: `_show_countin` popup (battute 1–8 + spunte), stato `_countin_on/_countin_beats/_countin_loop`, armato in `_on_play`, gated sull'analisi; **fix popup «Python-CFFI error»**: la callback audio di `mixer_engine.py` è avvolta in try/except (`_render`) così nessuna eccezione sfugge a cffi, con log una-tantum, e lo `stop()` ora chiude lo stream (niente callback attiva per ore da fermo); test nuovi count-in in `test_mixer_engine.py`; 1.10.0: tre funzioni Mixer/export + `other` per sottrazione: **accordi che seguono la trasposizione** — `_refresh_chords` in `ui_mixer.py` traspone le radici di `pitch_slider.value()` semitoni e rinfresca dopo ogni `_apply_transform`; **export «Foglio accordi» .txt** — nuovo radio `rb_sheet` in `ExportOptionsDialog` (gated su analisi presente), `_build_chord_sheet`/`_export_chord_sheet` scrivono accordi per sezione + tonalità/BPM, trasposti e nella notazione corrente, helper `_note_to_pc`/`_PC_ALIASES` per la tonalità in intestazione; **export senza metadati** — spunta `chk_no_meta`, propagata fino a `_export_audio` che aggiunge `-map_metadata -1 -bitexact` a ffmpeg per MP3 puliti (i WAV sono già senza tag); **stem `other` per sottrazione in rof6** — `stems._other_by_subtraction` calcola `other = strumentale − (drums+bass+guitar+piano)` così la somma dei 5 stem ricompone il mix, con fallback all'`other` di Demucs se fallisce o formato mp3 (DA GIUDICARE ALL'ASCOLTO: la sottrazione riversa in `other` anche gli errori degli altri stem); 1.9.0: **pagina «Guida»** — nuovo `app/ui_guide.py`: manuale utente completo in HTML statico (QTextBrowser, ancore interne per l'indice, CSS dai token di `theme.py`, QSS `#GuideView`), registrata nel rail con icona `info` sopra Impostazioni (stack: 0 Scarica, 1 Mixer, 2 Testi, 3 Guida, 4 Impostazioni — gli indici Mixer/Testi restano dinamici); **seek da tastiera nel Mixer** — ←/→ spostano la playhead di ±1 s (auto-ripetizione QShortcut: tenendo premuto scorre), Shift+←/→ = ±5 s (`_nudge_seconds` in `ui_mixer.py`); 1.8.0: tre funzioni Mixer + supporto: **punti nominati** — marcatori e loop A-B salvati con nome per brano in `mix.json` (chiave `marks`), bandierine sulla timeline (`TimelineWidget.set_marks`), richiamo da menu «Punti» o Ctrl+1…9, richiamo di un loop imposta A-B e attiva il loop (`_recall_mark` in `ui_mixer.py`); **preset di mix** — nuovo `app/mix_presets.py` (5 predefiniti + preset utente in `%APPDATA%/Sonora/mix_presets.json`, per nome stem: applicare = neutro + override, così valgono su ogni brano), pulsante «Preset» nell'header del Mixer; **scalette** — nuovo `app/setlists.py` (`%APPDATA%/Sonora/setlists.json`), pulsante «Scalette» (crea/rinomina/elimina, aggiungi/rimuovi brano corrente), navigazione Ctrl+→/← con posizione nel bottone («Scaletta 2/7»), brano fuori scaletta la disattiva (`_sync_setlist_state`); **esporta diagnostica** — nuovo `app/diagnostics.py` + pulsante in Impostazioni → Motore stem: zip sul Desktop con report (versione/OS/licenza senza token/stato motore/nvidia-smi), settings.json e log rotanti, su QThread (`DiagnosticsWorker` in `ui.py`); test nuovi: test_diagnostics, test_mix_presets, test_setlists (129 totali); in repo anche CLAUDE.md e config Claude Code (`.claude/`: hook pytest+blocco cartelle generate, skill /release); 1.7.3: coda senza rielaborazioni: «Separa tutti» non propone più il popup «vuoi separarli di nuovo?» — separa solo i non separati, toast se non c'è nulla da fare, il redo del singolo brano resta nel menu contestuale; nuovo pulsante «Rimuovi completati» (`clear_done_btn`/`_on_clear_done` in `ui.py`, rimuove le righe «fatto»); `_on_download` esclude i file locali senza url; 1.7.2: barra separazione senza più riavvii: demucs stampa una barra tqdm per ogni passata interna (modelli del bag × `--shifts`, es. htdemucs_ft su GPU = 8 barre) e ora `stems._multipass_progress` le rimappa su un unico 0–100% monotono; percentuale visibile sulla riga coda e sul chip playbar (`task_update` aggiunge «· N%»); nel Mixer, metronomo e griglia beat cliccati senza analisi mostrano un toast esplicativo (`_needs_analysis` in `ui_mixer.py`, flag `_beats_ready` al posto del disable del pulsante Click); 1.7.1: fix crash ricerca testi (QThread), opzione «Dimensione interfaccia», coda compatta a vuoto, fix `last_seen_version`; 1.7.0: playbar contestuale: fissa solo su Testi, altrove appare solo con un task lungo in corso — `_update_playbar_visibility` in `ui.py`, segnale `task_state_changed` della PlayBar; progress sul totale nelle modalità multi-passo — `stems._step_progress` avvolge `progress_cb` per passo, rof6 = 3 passi, 6hq = 2; fallback OOM anche per Roformer — scaletta CUDA→segmenti corti (profilo "low" di `roformer_script.py`, `mdxc_params` con `segment_size` 128)→CPU (`CUDA_VISIBLE_DEVICES=-1` via `extra_env` di `_stream`), stessa logica già collaudata di `_run_demucs`; 1.6.0: Testi rifatti: karaoke rimosso, ricerca LRCLIB multi-strategia con pulizia titoli YouTube — `search_plan`/`clean_title` in `app/ui_lyrics.py` —, libreria locale dei testi in `%APPDATA%/Sonora/testi` (`app/lyrics_store.py`, pulsanti «Salva»/«Libreria», elimina col tasto destro), i vecchi `lyrics.lrc` vengono migrati in `lyrics.txt`; 1.5.5: fix: cartelle stem con `[]` nel nome (es. video YouTube "Titolo [ID] - stems") facevano fallire "Analizza" — `glob` in `analyze_script.py` trattava le parentesi quadre come character-class invece che testo letterale, ora `glob.escape()` sulla cartella; 1.5.4: fix "Analizza" non segnalava errore quando l'analisi tornava senza stem trovati — ora `stems.analyze()` solleva se il JSON contiene `error`; 1.5.2: Testi: ricerca artista/titolo separati su LRCLIB, cancellazione ricerca senza freeze, pulsante Esporta; ricerca video: anteprima audio (~20s) sui risultati prima di aggiungerli alla coda; 1.5.1: fix icone SVG mancanti nel pacchetto PyInstaller; 1.5.0: redesign completo UI, vedi sotto; 1.4.0: basi «senza una traccia»; 1.3.0: export stem separati/WAV-MP3 + niente auto-analisi; 1.2.0: menu modalità stem riorganizzato; 1.1.0: Roformer SW 6 stem + pulsanti ±semitono nel Mixer; allineata in `app/__init__.py`, `installer/sonora.iss` e GitHub).
...
e resta consultabile dal pulsante «Novità» in Impostazioni → Aggiornamenti.
...
- **Mixer / studio di pratica** (scheda integrata):
  ...
  - **Pre-conteggio "Conteggio"** (gruppo CLICK): al Play una battuta di click a tempo prima dell'audio (1–8 battute), con opzione «anche a ogni giro del loop». Richiede l'analisi.
  ...
  - **scorciatoie** (Spazio/L/Home/A/B/1-6, ←/→ ±1s con auto-ripetizione, Shift+←/→ ±5s).
